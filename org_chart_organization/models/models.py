...
class OrgChartDepartment(models.Model):
    ...
    _description = "Org Chart Department"
    ...
    def get_department_data(self):
        ...
        for department in departments:
            ...
            if department_data.manager_id.id:
                ...
                department_manager_name = self.acronym_name_string(
                    department_manager_name, type=True
                )
                department_manager_image = (
                    self.env["ir.config_parameter"].sudo().get_param("web.base.url")
                    + "/web/image/hr.employee/"
                    + str(department_data.manager_id.id)
                    + "/image/170x170",
                )
            data.append(
                {
                    ...
                    "num_employee": len(
                        ...
                        .search([("department_id", "=", department_data.id)])
                        .ids
                    ),
                }
            )
            ...
            if (
                department_data.parent_id.id
                and department_data.parent_id.id not in group_parent
            ):
    ...
    def get_department_child(self, root, level):
        ...
        departments = self.env["hr.department"].search(
            [("parent_id", "=", root.id), ("company_id", "=", self.env.company.id)]
        )
        for department in departments:
            ...
            if len(children_data) > 0:
                for e in children_data:
                    result.append(e)
        result.append({"department": root, "level": level})
        return result
    ...
    def acronym_name_string(self, string, type=None):
        ...
        if type is None:
            if len(aro_str.split(".")) > 3:
                ...
                new = (
                    new
                    + string.split()[len(aro_str.split(".")) - 2]
                    + "."
...
class OrgChartEmployee(models.Model):
    ...
    _description = "Org Char Employee"
    ...
    def get_employee_data(self):
        ...
        level = 0
        roots = self.env["hr.employee"].search(
            [("parent_id", "=", False), ("company_id", "=", self.env.company.id)]
        )
        for root in roots:
        ...
        for employee in employees:
            ...
            get_employee = self.env["hr.employee"].search(
                [("id", "=", employee_data.id)]
            )
            ...
            if (
                employee_data.parent_id.id
                and employee_data.parent_id.id not in group_parent
            ):
    ...
    def get_employee_child(self, root, level):
        ...
        for employee in employees:
            ...
            if len(children_data) > 0:
                for e in children_data:
                    result.append(e)
        result.append({"employee": root, "level": level})
        return result
