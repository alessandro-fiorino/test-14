odoo.define('web_image_sign.web_image_sign', function (require) {
    "use strict";
    ...
    var FieldImageSignature = BasicFields.FieldBinaryImage.extend( {
        template: 'FieldImageSignature',
        ...
        jsLibs: ['/web_image_sign/static/lib/jSignature/jSignatureCustom.js'],
        ...
        start: function () {
            var self = this;
            this.$(".image_signature").jSignature("init", this.sign_options);
            this.$(".image_signature").attr( {
                "tabindex": "0",
                'height': "100",
            });
            this.empty_sign = this.$(".image_signature").jSignature("getData",
                'image');
            self._render();
        },
        _on_clear_sign: function () {
            this.$(".image_signature > canvas").remove();
            this.$('> img').remove();
            this.$(".image_signature").attr("tabindex", "0");
            var sign_options = {
                'decor-color': '#D1D0CE',
                'color': '#000',
                'background-color': '#fff',
            ...
            this.$(".image_signature").jSignature(sign_options);
            this.$(".image_signature").focus();
            this._setValue(false);
        },
        _on_save_sign: function (value_) {
            var self = this;
            this.$('> img').remove();
            var signature = this.$(".image_signature").jSignature("getData", 'image');
        ...
        _render: function () {
            ...
            if (this.mode === "readonly") {
                ...
                if (this.value) {
                    ...
                    self._rpc( {
                        ...
                        if (data) {
                            var field_desc = _.values(_.pick(data[0],
                                field_name));
                            self.$(".image_signature").jSignature("clear");
                            self.$(".image_signature").jSignature("setData",
                    ...
                    this.$(".image_signature").jSignature("init", sign_options);
                }
            } else if (this.mode === 'create') {
                this.$('> img').remove();
                this.$('> canvas').remove();
                if (!this.value) {
                    this.$(".image_signature").empty().jSignature("init", {
                        'decor-color': '#D1D0CE',
                        'color': '#000',
                        'background-color': '#fff',
                        'height': '150',
    ...
    Registry.add('image_signature', FieldImageSignature);
